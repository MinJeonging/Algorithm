...
2. 문제를 풀기 위한 접근
   - DP(동적 계획법)을 사용하여 점화식으로 문제 해결.
   - dp[i][j]: i자리 계단 수 중 마지막 자리가 j인 경우의 수.

3. 점화식 도출
   - dp[i][j]는 i-1자리 계단 수 중 마지막 자리가 j-1 또는 j+1인 경우에서 결정됨.
...
5. 구현 순서
   - DP 배열 초기화:
     - dp[i][j]를 0으로 초기화하고, dp[1][j]를 초기 조건에 맞게 설정.
   - 점화식에 따라 DP 배열 채우기:
     - 각 자리 수(i)와 마지막 자리(j)에 대해 점화식을 이용하여 계산.
   - 결과 출력:
     - dp[N][0] ~ dp[N][9]의 합을 계산하여 10^9로 나눈 나머지를 출력.
"""

# 입력 처리
N = int(input())  # 계단 수의 자리 수 입력
